...
def callback(data_re):
	...
	if not processing:
		...
		#print(msg_array)  #msg_array= coordenadas en float32  [[(-0.1, 0., 0.1)]]
		# Muestreo cada valor de paso_resample del pointcloud 
		msg_resize = msg_array[:,1:msg_array.shape[1]:paso_resample] #Inicia apartir del segundo arreglo o xyz encima paso_resample muestra los valores de 1 
        #en 1 dependiendo del paso de este si es , es decir resample es la variable que sirve para indicar cuantas coordenadas se van a ignorar
        #proporcionalmente dependiendo de la cantidad de esta
		# Mostrando la informacion de la dimension actual de los puntos por laser
		#rospy.loginfo(msg_resize.shape[1]*msg_resize.shape[0])  #Multiplica las dimensiones 1,1 dependiendo del resample , es decir edita el tamano
		# Funcion que convierte el array a pc2  
		data_pc2 = array_to_pointcloud2(msg_resize, header=data_re.header) #convierte el array msg_resize
...
def listener():
	...
	rospy.Subscriber("/data_cloud", PointCloud2, callback)
